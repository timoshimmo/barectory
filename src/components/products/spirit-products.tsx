...
export default function SpiritProductsGrid({
  className,
  limit = 30,
  variables,
}: Props) {
...
//  const { products, isLoading, error } = usePopularProducts(variables);
  ...
  const { products, isLoading, error } = useCategoryProduct({ slug: 'spirits' });
  ...
  return (
    <SectionBlock>
      <div className={classNames(className, 'w-full relative')}>
        <Link
          href='grocery/search?category=spirits'
          className={cn(
        ...
        >
          <span className="whitespace-nowrap text-3xl">Spirits</span>
        </Link>
        <Swiper
